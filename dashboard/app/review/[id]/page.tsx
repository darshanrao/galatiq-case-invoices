...
function riskColor(score: number) {
  if (score >= 0.7) return "text-red-300 bg-red-900/50";
  if (score >= 0.4) return "text-yellow-300 bg-yellow-900/50";
...
export default async function ReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-zinc-900 p-8 text-gray-100">
      <div className="max-w-2xl mx-auto">
        <a href="/" className="text-sm text-blue-400 hover:text-blue-300 hover:underline mb-6 inline-block">
          ← Back to dashboard
        </a>

        <div className="bg-zinc-800 rounded-2xl border border-zinc-700 p-8 shadow-sm">
          <div className="flex items-start justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-100">{review.invoice_number}</h1>
              <p className="text-gray-500 mt-1">{review.vendor}</p>
            </div>
            <div className="text-right">
              <p className="text-3xl font-bold text-gray-100">${review.amount.toLocaleString()}</p>
              <span
          ...
          <div className="space-y-4 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-300 leading-relaxed">{review.flag_explanation}</p>
            </div>
          ...
          {isPending ? (
            ...
            <div className="rounded-xl border border-zinc-700 p-5 bg-zinc-800/50">
              ...
              {review.decision_reasoning && (
                <p className="text-sm text-gray-400 mt-2">{review.decision_reasoning}</p>
              )}
